...
- **依赖**：[RFC-0001](RFC-0001-Architecture.zh-CN.md)、[RFC-0002](RFC-0002-ANA-Chain.zh-CN.md)、[RFC-0003](RFC-0003-Envelope.zh-CN.md)、[RFC-0004](RFC-0004-Memory-System.zh-CN.md)
...
一个最小 ANA Local Agent Runtime 必须逻辑上包含以下组件。
...
Router 必须以能力匹配为主。选择算法可以考虑本地可用性、延迟、成本、用户偏好或信任设置，但不得因此绕过 Safety Policy。

Adapter 的职责：`ANAEnvelope → Provider 原生请求`，`Provider 原生响应 → ProviderResult + ProposedActions`。Adapter 必须保持这条边界，不得直接调用 Executor 或以 Provider 结果覆盖本地策略。

## 7. Executor

Executor 只接收已经通过 Safety Policy 的结构化动作。它必须验证动作目标仍处于已授权范围，执行后返回结果，不接受未经策略判定的自由文本命令作为授权。

## 8. Safety Policy

Safety Policy 是 Local Runtime 的最终行为门。它至少应能返回：`allow`、`confirm`、`sandbox`、`deny`。无论 Provider 是官方、第三方或本地模型，Safety Policy 都必须保留否决权。

## 9. 运行时状态同步

Runtime 使用 RFC-0002 的 ANA Chain 传递任务事件、Memory 增量、策略决定和工具结果。v0.1 不规定自治 Agent 之间的复杂谈判或冲突自动合并。
