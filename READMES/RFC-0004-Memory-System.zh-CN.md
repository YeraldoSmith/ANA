...
- **依赖**：[RFC-0001](RFC-0001-Architecture.zh-CN.md)、[RFC-0003](RFC-0003-Envelope.zh-CN.md)
...
Semantic Memory 是经过验证或持续复用的高层结论。它必须保留足以追溯其来源或更新理由的信息，且允许被用户纠正。

### 3.4 User Preference

User Preference 记录用户明示或经确认的工作偏好。它不是不可见画像；用户应能检查、编辑、导出和删除。

### 3.5 Project State

Project State 表示与一个明确项目或任务相关的状态。它必须与一般用户偏好分开，以避免一个项目的临时状态错误地影响其他项目。
...
每次演化都是有意抽象，而非 ANA Chain 的可逆变换。实现应保留其输入来源、产生时间、产生者、适用范围和可撤销关系。

## 5. 可移植性与迁移

- `portable`：用户明确可跨设备带走的偏好、语义结论或兼容项目状态
- `device_local`：本地路径、硬件探测、模型缓存等不可直接迁移状态
...
3. 向 Provider 披露前，Runtime 应优先使用必要的结构化或摘要形式。
