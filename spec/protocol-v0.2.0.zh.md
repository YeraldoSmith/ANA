...
- CRC-16 + HMAC 双重校验架构（检错+防篡改分离）
- CODON ACK 确认 + 超时重传（ACKTracker）
- PING/PONG 心跳对端存活检测
- 子链同步校验（chain_index 嵌入 CODON 包）
- 控制密码子：PING/PONG/TEARDOWN
- CODON 包新增 chain_index 和 ACK 标志位
- 新增错误码：SUBCHAIN_MISMATCH、PEER_UNREACHABLE
- 测试从 62 → 91 项

## v0.1.0 (2026-08-03)
- 初始草案
- 核心线路格式定义 + 协商流程
- 码本层级结构（Service→Operation→Template）
- 噪声密码子机制 + 数据分片
- 安全威胁模型 + JSON/Protobuf/ANA 对比
- 术语表 + 超时与重试 + 参考实现
```

---

*规范版本: v0.2.0 | 日期: 2026-08-04*
