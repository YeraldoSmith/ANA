# ANA Chain Protocol Specification v0.1
...
Packets are padded to the nearest standard size: 64, 128, 256, 512, or 1024 bytes. Padding bytes are random (also serving as anti-traffic-analysis noise). Padding is appended **after** the CRC-16 checksum, so it does not affect `payload_len`.
...
### 5.2 Important Note on CRC-16

> ⚠️ **CRC-16 provides error detection, NOT tamper resistance.**

An active attacker can:
1. Intercept a CODON packet → modify payload → recompute CRC-16 → forward
2. Receiver's CRC check passes, anticodon lookup may resolve to a wrong operation

**If you need tamper resistance** (recommended):
- Add **HMAC-SHA256** authentication tag alongside CRC-16 (+32 bytes)
- Or rely on **TLS** for integrity protection (recommended for production)

### 5.3 Relationship to TLS
...
| Message integrity | ❌ (CRC-16 ≠ MAC) | ✅ (GCM auth tag) | Rely on TLS |
...
### 8.3 v0.2+ Planned Features

- **HMAC-SHA256 message authentication** — replaces CRC-16 for true integrity
- **Constant-time lookup** — resists timing side-channels
- **Constant-rate transmission** — fixed-interval sends to fully mask activity
- **Bloom filter pre-screening** — O(1) rejection of invalid codons (anti-DoS)
...
- **Tests**: 62 unit tests passing (HKDF RFC5869 vectors, ChaCha20 PRNG, varint codec, packet serialization, noise filtering, anti-replay, sub-chain rotation, session lifecycle)
...
├── packet.py      # Packet format + CRC-16
...
├── reliability.py # ACK tracking + retransmission + heartbeat + chain sync
...
## v0.1 (2026-08-04)
- Initial draft
- Core wire format defined
- Negotiation flow specified
- Codebook hierarchical structure
- Noise codon mechanism
- Data fragmentation mechanism
- Security threat model (complete)
- JSON/Protobuf/ANA comparison
- Glossary
...
*Specification version: v0.1 | Date: 2026-08-04*  
*Review edition — P0/P1 items added (noise mechanism, security model, glossary, timeout/retry, reference impl)*
