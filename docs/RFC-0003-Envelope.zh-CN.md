...
- **依赖**：[RFC-0001](RFC-0001-Architecture.zh-CN.md)、[RFC-0002](RFC-0002-ANA-Chain.zh-CN.md)、[RFC-0004](RFC-0004-Memory-System.zh-CN.md)
...
Envelope 在进入 ANA Chain 前必须被编码为 canonical JSON：UTF-8、对象键按 Unicode 代码点升序排列、数组顺序保持原样、无多余空白、明确的 `version`。Envelope 版本仅描述任务对象 schema；它不同于 Chain 版本和 Codon 词典版本。
...
该 profile 不对 `intent`、`capabilities` 或 `policy` 建立超出测试向量的全局词表。

**Phase 7 clarification：**本 profile 的必需 Envelope 字段仅为本节已列出的字段；v0.1 不定义"未知字段但对所有接收方仍为必需"的标记。未知成员因此只能作为可忽略或可保留的扩展，不能改变 v0.1 Core 语义。
